...
func (pq *PlaylistQuery) QueryUploads() *UploadQuery {
	query := &UploadQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlaylistQuery) Clone() *PlaylistQuery {
	...
	return &PlaylistQuery{
		...
		withUploads: pq.withUploads.Clone(),
...
func (pq *PlaylistQuery) WithUploads(opts ...func(*UploadQuery)) *PlaylistQuery {
	query := &UploadQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withUploads = query
	return pq
}
...
func (pq *PlaylistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Playlist, error) {
	...
	if query := pq.withUploads; query != nil {
		if err := pq.loadUploads(ctx, query, nodes,
			func(n *Playlist) { n.Edges.Uploads = []*Upload{} },
			func(n *Playlist, e *Upload) { n.Edges.Uploads = append(n.Edges.Uploads, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlaylistQuery) loadUploads(ctx context.Context, query *UploadQuery, nodes []*Playlist, init func(*Playlist), assign func(*Playlist, *Upload)) error {
	...
	query.Where(predicate.Upload(func(s *sql.Selector) {
		s.Where(sql.InValues(playlist.UploadsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
